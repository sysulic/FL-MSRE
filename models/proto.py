...
class Proto(fewshot_re_kit.framework.FewShotREModel):
    ...
    def __init__(self, encoder, dot=False):
        fewshot_re_kit.framework.FewShotREModel.__init__(self, encoder)
    ...
    def forward(self, support, query, N, K, total_Q):
        ...
        # Ignore NA policy
        support = torch.mean(support, 2) # Calculate prototype for each class, 计算原型 -> 对每一类的样本 embedding 取均值 -> (B, N, D)
        logits = self.__batch_dist__(support, query) # (B, total_Q, N)
        minn, _ = logits.min(-1)  # 选取距离最近的??
