...
class BERTSentenceEncoder(nn.Module):
    ...
    def forward(self, inputs):
        _, x = self.bert(inputs['word'], attention_mask=inputs['mask']) # x对应的是[CLS]的隐含向量
        return x
    
    def tokenize(self, text):
        # text -> the input of bert
        text = self.tokenizer.tokenize(text)
        text = text[:self.max_length - 2]
        input_sequence = ["[CLS]"] + text + ["[SEP]"]
        indexed_tokens = self.tokenizer.convert_tokens_to_ids(input_sequence)
        indexed_tokens += [0] * (self.max_length - len(input_sequence))
        mask = [1] * len(input_sequence) + [0] * (self.max_length - len(input_sequence))
