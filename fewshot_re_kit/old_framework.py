...
class FewShotREModel(nn.Module):
    ...
    def accuracy(self, pred, label, multi_choose=1):
        '''
        pred: Prediction results with whatever size
        label: Label with whatever size
        return: [Accuracy] (A single value)
        '''
        # 多数投票，因为 Q = 1，所以label相同的必定为同一类
        Q = pred.shape[-1] / multi_choose
        Q = int(Q)
        result = pred.view(-1) == label.view(-1)
        acc = torch.zeros((Q,), dtype=torch.bool)
        begin = 0
        for i in range(Q):
            score = torch.mean((result[begin:begin + multi_choose]).type(torch.FloatTensor)).item()
            if score > 0.5:
                acc[i] = True
            else:
                acc[i] = False
...
class FewShotREFramework:
    ...
    def train(self,
        ...
        if bert_optim:
            ...
            optimizer = AdamW(parameters_to_optimize, lr=2e-6, correct_bias=False)
            if self.adv:
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            if (it + 1) % val_step == 0:
                ...
                acc = self.eval(model, B, N_for_eval, K, Q, val_iter, 
                        na_rate=na_rate, pair=pair, multi_choose=multi_choose)
                model.train()
                if acc > best_acc:
        ...
        if ckpt is None:
            ...
            for name, param in state_dict.items():
                ...
                try:
                    ...
                    print(name,  "is different from current framework")
                    continue
            ...
            for it in range(eval_iter):
                ...
                if torch.cuda.is_available():
                    ...
                    for k in query:
                        query[k] = query[k].cuda()
                    label = label.cuda()
                # logits, pred = model(support, query, N, K * multi_choose, Q * N * multi_choose)
                logits, pred = model(support, query, N, K * multi_choose, 1)

                right = model.accuracy(pred, label, multi_choose=multi_choose)
                with open('./bad_case_pred.txt', 'a', encoding='utf-8') as f:
                    f.write(str(pred.cpu().numpy()[0]))
                    f.write('\n')
